...
> **受众**：Monorepo 维护者（本地手测 ACP surface、多轮 HITL、权限弹窗等）。  
> **不随模板下发**：`packages/deepagents-flow-ts/` 与 `dev-agent-flow` **不得**引用本页；模板使用者日常开发用 `pnpm flow`（见包内 [README.md](../../../packages/deepagents-flow-ts/README.md)）。
...
以上都是**维护者本地 ACP 手测**（Zed 连 `tsx` 跑源码）。安装到目标机器或生产环境时，跑的是 **esbuild 自包含 bundle**（`dist/bundle.mjs`）——不依赖 `node_modules`、不需要 `tsx`。
