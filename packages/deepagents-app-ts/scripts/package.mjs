#!/usr/bin/env node
/**
 * Cross-platform package script (PowerShell / bash).
 */
import { createHash } from "node:crypto";
import { spawnSync } from "node:child_process";
import { copyFileSync, mkdtemp, mkdir, readFileSync, rm, writeFileSync } from "node:fs";
...
async function main() {
  ...
  const npmCache = process.env.NPM_CONFIG_CACHE || path.join(tmpdir(), "deepagents-app-ts-npm-cache");
