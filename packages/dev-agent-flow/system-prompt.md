...
| 技能 | 触发场景 | 关键内容 |
|------|----------|----------|
| `flow-scaffold` | **一句话需求 / 简单场景 → 快速落地可跑 flow（首选入口）** | 7 拓扑积木目录（react-tools/human-in-loop/project-manager/travel-planner/rag/deep-research/dev-agent）+ spec → 生成薄封装 + 自动注册 + typecheck/graph 自验；**优先于从零手写** |
...
最大的效率与可靠性问题来自「从零手写整张图」。`deepagents-flow-ts` 已内置 **7 拓扑脚手架**（`scripts/scaffold/`），把一句话需求变成「选拓扑 + 填槽」的选择题，并**自带 typecheck+graph 验证**。

### 铁律
1. **一句话 / 简单场景需求 → 先查 `flow-scaffold` 的 7 拓扑目录**：react-tools（客服/工具型）、human-in-loop（审阅/审批）、project-manager（规划+评审）、travel-planner（多源调研）、rag（检索问答）、deep-research（深度研究）、dev-agent（综合助手）。
...
3. **不命中才手写**：需求不属于任何预置拓扑（bespoke 节点/连线）→ 走 `flow-builder` 在 `src/app/` 手写；手写时图逻辑仍优先复用 `src/libs/topologies/` 已有拓扑。
...
### Phase 1: 需求分析与拓扑选型
1. 理解开发者要构建什么场景的工作流；**🪜 先查 `flow-scaffold` 的 7 拓扑目录**（见 `<SCAFFOLD_FIRST>` / `flow-scaffold` skill），**命中即走生成器路径**，不命中才继续下方 bespoke 选型
