...
- **平台能力**：**写图前**先 `search-apis` / `search-skills` / `get-config`（tools·mcpConfigs·skills）并 `add-tool`；**登记即接入 —— 平台工具（Plugin/Workflow/MCP）运行期统一转 MCP 经 ACP `session/new` 的 `mcpServers` 下发**（合并 `mcp.default.json`，session-wins），default ReAct 自动 bind；**禁止**手写 fetch 包装已登记能力；收工须贴搜索证据；内置 grep/glob **仅工作区**（**联网搜索较常见**，规则相同）→ Part 3
...
**原则**：先查 Skill 再动手；`README` + `glossary` 为项目权威；`examples/` 只读且只参考 surface seam，节点/边以 `src/libs/topologies/` 为准；LangGraph TS API 查官方文档；平台能力禁止凭记忆填 `targetId`。
...
收到 flow 需求先答**第 0 问**（`flow-builder` Part 0 § Phase 1）：多轮对话/**追问**/钻取/泛化 → **零图路径**（`activeFlow: "default"` ReAct + 平台能力登记 + systemPrompt，不写图）；固定管道/HITL → **Part 1**（9 topologies = 8 presets + `custom`）。**凡需平台工具/MCP/技能/Plugin 须先 Part 3 搜平台登记**（见 Part 0 § 平台能力门禁）。命中 preset **禁止**手写图；不命中先用 `custom`。系统提示词与 scaffold 并行 → Part 5。
</SCAFFOLD_FIRST>
...
5. 命中 → `add-tool.sh` → **登记即接入**（运行期自动成为 MCP 工具，见下）→ 记入 `project.md`
...
**运行期统一 MCP 下发（关键模型）**：平台已登记的一切工具能力（**Plugin / Workflow / MCP**）运行期由平台后端**统一转成 MCP**，经 ACP `session/new` 的 `mcpServers` 下发，runtime 合并进 `allTools`/`ctx.mcpTools`（与 `config/mcp.default.json` 合并，默认 `session-wins`）。**conversational ReAct（default flow）`think ↔ tools` 自动 bind，零代码可用**；固定管道用 `createMcpRetrievalNode` / 工具名引用（`searchMcp` / `docMcp`）。**禁止**为已登记平台能力手写 fetch / `tool()` 包装（schema 仅用于理解参数含义）；**禁止**运行时代码调用 `4sandbox` 系平台内部端点（仅 dev 脚本可用）。

**常见专项 · 联网搜索**：需求含互联网/实时/网页检索时，在通用流程上追加 `搜索`/`联网`/`web` 关键词并查 `mcpConfigs`（`mcp-retrieval` / `searchMcp`）→ Part 3 § 联网搜索 · § 运行期统一 MCP 下发。完整步骤 → Part 3 § 平台能力登记。
</PLATFORM_CAPABILITIES>

<WEB_SEARCH>
## 联网（约束 · 常见专项）

**联网搜索是平台能力登记中最常见的场景之一；运行期经 ACP 以 MCP 提供，模板不内置。** 需要互联网/实时/网页搜索/多源调研时：先走 `<PLATFORM_CAPABILITIES>` 通用流程，追加 `搜索` / `联网` / `web` 关键词并查 `mcpConfigs`；**登记即接入** —— 运行期平台把命中的 Plugin/MCP 统一转成 MCP 经 ACP `mcpServers` 注入，**default ReAct 自动 bind（首选，零代码）**；固定管道才用 `mcp-retrieval` / `searchMcp` 按名接线。**禁止**照 Plugin schema 手写 fetch 搜索工具（失败案例：猜端点 + 猜 envelope + 无超时 → 运行期卡住/全空）。步骤 → Part 3 § 平台能力登记 · § 联网搜索 · § 运行期统一 MCP 下发。
</WEB_SEARCH>
...
**工具优先级**（外部/业务能力）：平台能力（登记后运行期即 MCP 工具，零包装）→ `libs/tools` 内置（grep **仅工作区**）→ 自写 `src/app/`（最后手段，仅平台确无命中的真外部 API）。详表 → Part 3。
</CODE_FORMAT_RULES>
...
12. **为已登记平台能力手写 fetch / `tool()` 包装**（运行期它们已是 MCP 工具） 13. **运行时代码调用 `4sandbox` 系平台内部端点**（仅 dev-engineer-toolkit 脚本可用；端点/envelope 一律不得猜测）

**关键注意**：平台已登记工具能力（Plugin/Workflow/MCP）**运行期统一转 MCP 经 ACP `session/new` 的 `mcpServers` 下发**（合并 `mcp.default.json`，session-wins）；模板 `mcp.default.json` **仅内置 `ask-question`**（结构化提问 fallback，图内 HITL 表单即 **平台问答卡片**，见目标项目 `docs/glossary.md`），禁止内置搜索/文档包；**default ReAct 禁止在 think 里自发调 `nuwax_ask_question`**——审阅定稿走 `human-in-loop` 的 `present_review` + `review`；`activeFlow` 拼错/未注册会 **warn 后回落 default**（注册表见 `src/app/flows/index.ts`）；默认图无凭证 fallback；`createStatefulFlow` + `durableCheckpointer`；`permissions.interruptOn` 工具审批在 `config/`（非平台）。
</DEVELOPMENT_CONSTRAINTS>
...
**Phase 4 脱敏与 ACP 默认集成**（收工报告强制）：
- **禁止**出现沙箱/平台环境变量名；**禁止**要求用户配置平台 API 基址、沙箱认证、项目标识等（ACP 沙箱已默认注入，无需说明）。
- **禁止**把 `add-tool` / Plugin Authorization / API key / MCP 登记写成「用户后续」；开发期应自行完成。
