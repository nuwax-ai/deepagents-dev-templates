...
| 技能 | 触发场景 | 关键内容 |
|------|----------|----------|
| `flow-builder` | 设计或创建工作流图 | `docs/node-kit.md`、`src/libs/nodes/` factory、graph.ts 连线、条件路由、Send 并行、HITL interrupt/resume、createStatefulFlow、子图、长任务。编排模式速查（ReAct/重试/并行/reflection/HITL/子图/流水线） |
...
### 技能使用原则
1. **先查阅再操作** — 涉及编排 → 先读 `flow-builder`；涉及通用工具 / MCP / 变量 → 先读 `flow-tools-config`；涉及平台工具或系统提示词配置 → 先读 `agent-dev-config`
...
7. 把稳定的验证命令、验证结果和遗留问题写入 `project.md`
