...
| 技能 | 触发场景 | 关键内容 |
|------|----------|----------|
| `flow-scaffold` | **一句话需求 / 简单场景 → 快速落地可跑 flow（首选入口）** | 8 拓扑积木（7 预设 react-tools/human-in-loop/project-manager/travel-planner/rag/deep-research/dev-agent + `custom` 任意节点级编排）+ spec → 生成薄封装 + 自动注册 + typecheck/graph 自验；**优先于从零手写** |
...
| `flow-prompt-designer` | 设计 / 重写目标 Agent 的系统提示词或开场白 | 七要素骨架、客服 / 内容生成 / 数据分析 / 任务工具型 场景 few-shot 模板、质量自检；经 `agent-dev-config` 保存并回读校验 |
...
### 技能使用原则
1. **先查阅再操作** — 涉及编排或运行时排查 → 先读 `flow-builder`（调试日志见 **Step 7**）；涉及通用工具 / MCP / 变量 → 先读 `flow-tools-config`；涉及平台工具或系统提示词配置 → 先读 `agent-dev-config`；涉及目标 Agent 提示词设计 / 重写 → 先读 `flow-prompt-designer`
...
### 铁律
1. **一句话 / 简单场景需求 → 先查 `flow-scaffold` 的拓扑目录**：7 预设——react-tools（客服/工具型）、human-in-loop（审阅/审批）、project-manager（规划+评审）、travel-planner（多源调研）、rag（检索问答）、deep-research（深度研究）、dev-agent（综合助手）；外加 `custom`（预设都不匹配时按 nodes/edges/state 自由编排，仍生成真实 TS）。
2. **命中拓扑 → 走生成器，不要手写图**：写 spec（`name/topology/systemPrompt/params`）→ `node scripts/scaffold/generate.mjs <spec>` → 改 `config` 的 `activeFlow`。生成器自动写薄封装 + 注册 + 跑 typecheck+graph（自带 COMPLETION_GATE）。
3. **预设不命中 → 先用 `custom`，而非直接手写**：7 预设都不匹配 → 用 `custom` 拓扑按 nodes/edges/state 自由编排（仍走生成器、生成真实 TS、自带验证）；**仅当 `custom` 也表达不了**（llm-stream/tool-exec/subgraph/自定义 reducer）才走 `flow-builder` 在 `src/app/` 手写（图逻辑仍优先复用 `src/libs/topologies/`）。
4. **目标 Agent 提示词**：scaffold 的 `systemPrompt` 槽位由 `flow-prompt-designer` 产出（七要素 + few-shot），再填进 spec。

> 详见 `flow-scaffold` skill（拓扑目录、spec schema、生成流程、注入规则、vs flow-builder）。这是治「跑不起来」（自验）+「过程慢」（选择题 vs 作文题）的关键。
</SCAFFOLD_FIRST>
...
4. 读取项目根目录 `project.md`；如果不存在，则基于 README、配置文件和当前代码创建
5. 如果任务涉及数据表、SQL API、平台工具或变量配置，先在 `project.md` 中定位已有记录，缺失则在本次完成后补齐
6. 若排查运行时问题，加载 `flow-builder` Step 7 并按其读 `.logs/`

### Phase 1: 需求分析与拓扑选型
1. 理解开发者要构建什么场景的工作流；**🪜 先查 `flow-scaffold` 的拓扑目录（7 预设 + `custom`）**（见 `<SCAFFOLD_FIRST>` / `flow-scaffold` skill），**命中即走生成器路径**；预设不命中先用 `custom`，再不行才走下方 bespoke 手写
...
### Phase 2: 开发实现
**🪜 命中预置拓扑（Phase 1）→ 走 `flow-scaffold` 生成器**：写 spec（`name/topology/systemPrompt/params`）→ `node scripts/scaffold/generate.mjs scripts/scaffold/specs/<name>.flow.json` → 改 `config/flow-agent.config.json` 的 `activeFlow` → 验证（详见 `flow-scaffold` skill）。**跳过下方手写步骤**。
...
3. 运行时改动按 `flow-builder` Step 7 读 `.logs/` 无未预期 `error`
...
- [ ] 运行时改动：按 `flow-builder` Step 7 读 `.logs/` 无未预期 `error`

> 命令语义与日志六步排查见 `flow-builder` Step 7（强制，非可选）。
</COMPLETION_GATE>
