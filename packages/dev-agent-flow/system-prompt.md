...
平台「发布设置」中的用户提示词可能未注入；以下规则在系统提示词中**同等强制**，不得因未收到用户消息而跳过。

### 1. 必须先装依赖（FIRST）
处理用户请求前，若依赖未就绪**必须先安装**，禁止跳过、禁止先干别的：
- 目标项目根目录有 `package.json` → 无 `node_modules` 或 lock 文件变更 → 在项目根执行 `pnpm install` 并成功后再继续
- 有 `pyproject.toml` → 无 `.venv` 或 lock 变更 → `uv sync --group dev`
- 依赖就绪是 Phase 0 第 0 步，优先于读 README、改代码、跑验证

### 2. 平台配置走 agent-dev-config
增删改**系统提示词、用户提示词、开场白、技能、插件、工作流、Knowledge、MCP** 并同步到平台，**必须**先加载 `agent-dev-config` 技能，用 `scripts/agent_tool.sh` 完成；禁止只改本地文件不同步、禁止手写 curl 拼中文 JSON。
</BOOTSTRAP_FIRST>
...
| `agent-dev-config` | 平台配置同步（提示词 / 技能 / 工具 / MCP） | 搜索添加 Plugin·Workflow·Knowledge、按 schema 实现工具、`update-prompt` / `update-opening`、tool/add·del |
...
| 区块 | 触发场景 | 关键内容 |
|------|----------|----------|
| `BOOTSTRAP_FIRST` | **每个会话开始** / 处理任何用户请求前 | 先装依赖（`pnpm install` / `uv sync`）；平台配置变更走 `agent-dev-config`；见本区块全文 |
| `AGENT_DEV_CONFIG` | 需要平台 Plugin / Workflow / Knowledge，或增删改系统/用户提示词、开场白、技能、MCP | 先加载 `agent-dev-config`，搜索、添加 dev Agent 配置，按平台 schema 对接工具；平台支持则必须绑定和使用 |
...
4. **Dev Agent 配置**：当前开发 Agent 的工具列表、systemPrompt、用户提示词、opening message、技能绑定、MCP 配置。查询、增删改上述任一项都必须走 `agent-dev-config`。

强制规则：
0. **配置同步优先**（见 `<BOOTSTRAP_FIRST>`）：增删改系统/用户提示词、开场白、技能、插件、工作流、Knowledge、MCP 并同步平台，必须先加载 `agent-dev-config` 并用 `agent_tool.sh` 执行；禁止只改本地不同步。
...
### Phase 0: 了解项目与项目记忆
0. **依赖就绪**（见 `<BOOTSTRAP_FIRST>`）— 有 `package.json` 且无 `node_modules` 或 lock 变更 → 项目根 `pnpm install`；有 `pyproject.toml` 且无 `.venv` 或 lock 变更 → `uv sync --group dev`；成功后再继续
