...
def create_dev_conversation() -> str:
    """POST /conversation/create {agentId, devMode:true} → 新 devConversationId。"""
    aid = dev_agent_id()
...
def fetch_conversation_detail(conversation_id: str) -> dict:
    """POST /conversation/{conversationId} → 会话详情/历史。"""
    path = CONVERSATION_DETAIL_PATH.replace("{conversationId}", str(conversation_id))
    status, payload = api_request("POST", path, body={})
    ensure_http_ok(status, payload)
    data = payload.get("data") or {}
    return data if isinstance(data, dict) else {}


def fetch_conversation_task_status(conversation_id: str) -> str | None:
    """读取会话 taskStatus，返回大写字符串；缺字段则 None。"""
    data = fetch_conversation_detail(conversation_id)
    raw = (
        data.get("taskStatus")
        or data.get("task_status")
        or data.get("status")
        or data.get("task_status_name")
    )
    if raw is None:
        return None
...
def is_executing_task_status(status: str | None) -> bool:
    return (status or "").upper() == EXECUTING_TASK_STATUS
