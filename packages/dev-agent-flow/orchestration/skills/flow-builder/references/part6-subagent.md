...
- **默认省略 `tools`** — 子 agent 继承父级工具（含当前会话的平台搜索能力，**不含 `task`**）；搜索工具 的 runtime 名由 `tools/list` 动态发现，无需写入 frontmatter
- **禁止** `tools: 联网搜索_1` 等平台登记名或中文 server 名 → 报 `配置了未知工具`
- **联网搜索**：平台登记的搜索能力运行期由运行环境提供；子 agent 可直接调用当前工具列表中已授权的搜索工具
- 框架 `task` 会在 AGENT.md 正文后自动追加委派约定（须非空最终结论、仅调用实际提供的工具）
- **复杂任务 Todo**：子 agent 使用内置 `write_todos` 提交完整清单快照；并行计划由 runtime 按父 `task` 的 `toolCallId` 合并，客户端中以 `[subagent]` 前缀区分
...
| 子 agent 调 平台搜索 401 | 检查平台搜索能力 的会话 Authorization 下发与连接复用 |
...
| 并行 Todo 相互覆盖 | runtime 应按父 `task` `toolCallId` 聚合后发送完整 客户端 `plan` 快照 |
...
```
主 Agent 联网检索 → 摘要写入 task.description
→ task({ subagent_type, description }) → 读 ToolMessage 结果 → 整合交付
```

- **并行多岗**：runtime 用 客户端 `messageId=subagent:<name>:<toolCallId>` 分桶流式预览；**主 Agent 可并行 `task`**
...
- [ ] 联网任务已登记平台搜索能力，运行期聚合 平台工具 的 `tools/list` 可见搜索工具
- [ ] smoke：`task` 至少一次成功（非 `Error:` / 非 `(subagent 无输出)`）
