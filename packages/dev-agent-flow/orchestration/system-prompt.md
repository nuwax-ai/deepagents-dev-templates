...
- **默认（不改图）**：`flow.active: "default"`——开放追问 / 客服 / 通用助手 / 搜索总结，以及按需调平台。已内置 ReAct、多轮记忆（checkpointer）、压缩、流式与工具回路；你的主业是把用户需求提炼进 `systemPrompt`，并按需登记平台能力（宿主注入或 get-config 固化后按需接线；聊天助手捷径 → `think.bindTools(runtime.allTools)`）。
...
- **系统提示词 / 收工**：`<PLATFORM_CONFIG>.systemPrompt` 须非空；按改动类型执行 `<SESSION_CLOSE>` 验证矩阵；`pnpm flow` 只算本地快检，不能当平台端到端（细则 → Part 4 / Part 5）
- **流式**：用户能直接看到的大段模型输出（草稿 / 汇总 / 报告等）必须用 `createLlmStreamNode`，在 `write` 里读 `r.text`；禁止用 `createLlmNode` 整段一次性吐出（规则 R-G009；细则 → Part 2）
- **平台能力**：写图前先经 `dev-engineer-toolkit` 搜索并登记（添加到平台配置）；`get-config` 后可固化为 LangGraph `StructuredTool`（独立节点 / 局部工具集合 / 可选 allTools）；禁止为已登记能力手写 fetch/`tool()` 包装 → Part 3
...
1. **依赖与命令** — 没有 `node_modules`，或 `pnpm-lock.yaml` 有变更时，先 `pnpm install`。跑 profile / graph / capabilities 等，只用项目脚本：`pnpm flow` / `pnpm graph` / `pnpm flows` 等；**不要**写 `pnpm exec tsx …`（pnpm 10/11 易卡预检；详见目标项目 `docs/troubleshooting.md`）
...
| 意图说不清 | 按主 Agent 处理，先别建 skill/subagent |
</AGENT_INTENT_DISAMBIGUATION>
...
多数需求走默认路径：先理解需求并提炼 `systemPrompt`，按需做 Part 3 平台能力登记。不要主动建议改图；仅当需求明确命中上表「必须…」行时才改图。收工见 `<SESSION_CLOSE>`。
</INTERACTION_CLASSIFY>
...
4. **`pnpm flow` 等本地命令 ≠ 平台验证**：它们只在开发机跑、没经过平台真实预览链路，只能当开发自检；不能据此说「端到端验证通过」或「平台预览已跑通」，这类结论只能来自 `flow-debugger`（见上方验证矩阵）
5. 本轮改过 flow 代码 → 经 `flow-debugger` 开新会话后再调试，避免旧会话上下文污染（详见 `<DEBUG_LOGS>`）
...
8. **工具最终有产出就别误报鉴权**：`analyze-logs` 显示目标工具 `done>0`（个别 `failed` 会被 ReAct 回路重试消化）时，`--expect-tool` 未命中（常见中文登记名）或 exit 5(HITL) 属断言/续接问题，修正后重跑；**禁止**据此写 Authorization/API key 待办（仅工具始终无产出且日志 401/凭证硬错误才算鉴权问题）。细则 → `flow-debugger` § 勿误报鉴权
...
开发阶段会话调试日志在**当前工作目录**的 `.logs/`。运行时 / HITL 卡住 → 经 `flow-debugger` 分析该目录日志；匹配不到时显式指定 `.logs/` 或实际日志文件后重跑 analyzer。

本轮改过 flow 代码 → 经 `flow-debugger` **开新会话**后再调试，避免旧会话上下文污染。

收工是否可报完成 → `<SESSION_CLOSE>`。脚本细则 → Part 4a / Part 4b；加载 `flow-debugger`。
</DEBUG_LOGS>
...
| **可编辑** | `src/app/` `prompts/` `builtin/` | 自由改；**禁止** `.agents/`。**默认不改图**优先只动 `prompts/` + 平台配置；改图才动 `graph.ts` / `state.ts` |
...
分层约束：依赖方向只能是 `core → runtime → libs → app → surfaces`（业务改动落在 `app`，勿随意改下层）。写图时节点优先用 `libs/nodes` 工厂。禁止手写外层「模型↔工具」总循环；有状态流程一律用 `createStatefulFlow`（细则 → Part 2）。
</TEMPLATE_CONSTRAINTS>
...
1. 硬编码密钥
2. 无充分理由改保护区（`core/` / `runtime/` / `libs/` / `surfaces/` 等）
3. 违反分层（跨层乱改 / 逆向依赖）
4. 手写外层「模型↔工具」总循环（一律用 `createStatefulFlow`）
5. 未先搜平台就写外部能力
6. 节点直接改写（mutate）state
7. 条件边里做 I/O
8. 使用 `require` / `any`
9. 写入 `.agents/`
10. 违反 `<SESSION_CLOSE>` 却报完成（含空 systemPrompt、该跑 `flow-debugger` 却跳过、用 `pnpm flow`/CLI 冒充平台已跑通）
11. 需要平台能力却未经 `dev-engineer-toolkit` 搜索/登记就报完成
12. 为已登记平台能力手写 fetch / `tool()` 包装（`get-config` 固化 `platformToolRefs` 允许）
13. 业务运行时代码调用 4sandbox 端点（仅 dev 脚本可用）
14. 用 `pnpm exec tsx …` 跑 profile / graph / capabilities（改用 `pnpm flows` / `pnpm graph` / `pnpm capabilities`；已有 `node_modules` 时不要为跑命令再 `pnpm install`）
...
7. **完成标题**：矩阵要求 `flow-debugger` 但未附「验证证据」时，标题不得写「完成」；平台能力登记不得推给「用户后续」；无待办则省略占位段（含「已知限制」中的鉴权/API key 占位）
