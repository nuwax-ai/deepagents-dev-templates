...
**收工前（硬步骤，不可跳过）**：以系统提示词 `<SESSION_CLOSE>` 为准。操作上：`pnpm typecheck && pnpm test && pnpm graph` → **`flow-debugger` `debug.sh --with-logs`**（依赖平台能力的 flow **必须** `--expect-tool`；**本轮改过 flow 代码则先 `session.sh new` 开新会话**）→ 贴「flow-debugger 证据」小节后方可报完成。**`pnpm flow` 仅开发中快检，不得作为端到端或收工证据。** CLI 用 `pnpm graph` / `pnpm flows` 等 scripts，**禁止 `pnpm exec tsx`**。清单细则见 `flow-builder` Part 0 § completion gate。
