...
| 目标 Agent 提示词基准 | `target-agent.base.md` | `flow.base.md`（经 `dev-engineer-toolkit` 同步 `<PLATFORM_CONFIG>`，定义见 `orchestration/system-prompt.md`） |
...
└── iteration/                    # 【迭代】台账 + 静态门禁（不下发）
```

> **分工**：`orchestration/system-prompt.md` = 规则与约束；`flow-builder/references/part0-workflow.md` = Phase 0–4 总流程；`part5-prompt-design.md` = 提示词提炼与平台同步；`dev-engineer-toolkit` = 配置读写脚本。

## 交付层（`orchestration/` · 配进编排后台）

人工同步到 **智能体开发 Agent** 编排后台的配置真源：
...
| [`orchestration/agent.manifest.json`](orchestration/agent.manifest.json) | 期望清单（对照用，非后台组件文件） |
...
| 源文件 | 目标目录 | 说明 |
|--------|---------|------|
| `orchestration/system-prompt.md` | `.claude/CLAUDE.md` 或 `.agents/system.md` | 系统提示词 |
| `orchestration/skills/*/SKILL.md` | `.claude/commands/` 或 `.agents/skills/` | 技能文件 |
...
| 层 | 内容 | 体量（约） | 加载时机 |
|----|------|-----------|----------|
| **L0** | `orchestration/system-prompt.md` + `user-prompt.md` | ~150 行 | 每次会话固定注入 |
| **L1** | `orchestration/skills/*/SKILL.md` | 每 skill ~80–120 行 | 按需 `load_skill` / 平台 skill 机制 |
| **L2** | `orchestration/skills/*/references/*.md` | 按 Part 单文件 | `Read` 或 `load_skill(name, part=…)` |

**原则**：
- `orchestration/system-prompt.md` 只保留铁律与边界；实现步骤在 `flow-builder` Part*
...
- **技能独立部署**：每个 `orchestration/skills/<name>/` 自包含（`SKILL.md` + `references/` + `scripts/`），**禁止**跨 skill 共享目录或 `source` 外部路径
