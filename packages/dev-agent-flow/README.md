...
| **开发 Agent 引导** | 本包 `system-prompt.md`（**规则/约束**）+ `skills/flow-builder/`（**实现步骤**）+ `skills/dev-engineer-toolkit/` | `system-prompt` 定铁律；`flow-builder` Part 0–7 承载逐步流程（脚手架、编排、persona 提炼、completion gate 等） |
...
> **分工**：`system-prompt.md` = 规则与约束；`flow-builder/references/part0-workflow.md` = Phase 0–4 总流程；`part5-prompt-design.md` = persona 提炼与平台同步；`dev-engineer-toolkit` = 配置读写脚本。

## 平台绑定（推荐 2 个 skill）

| 保留 | 职责 |
|------|------|
| `flow-builder` | Part 0 总流程；Part 1–4 脚手架/编排/工具/验证；Part 5 persona；Part 6–7 子智能体/技能 |
