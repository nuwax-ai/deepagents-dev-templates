...
4. **需要更新项目配置时** — 用户要求修改系统提示词、开场白、或任何项目级配置时，**必须**使用 `update-config.sh` 接口更新，不要仅本地修改文件。
...
更新智能体的系统提示词和开场白。**当用户要求修改系统提示词或开场白时，必须使用此脚本通过接口更新，不要仅本地修改文件。**

```bash
# 更新系统提示词
./scripts/update-config.sh --system-prompt "你是一个精通 Python 的开发助手..."
...
./scripts/update-config.sh \
  --system-prompt "你是一个专业的全栈开发助手..." \
  --opening-msg "你好！有什么可以帮你的？"

# 从文件读取（适用于长文本）
./scripts/update-config.sh --system-prompt-file "./new_prompt.txt"
./scripts/update-config.sh --opening-msg-file "./welcome.txt"
```
...
3. 执行 update-config.sh 写入新配置
4. 再次执行 get-config.sh 确认写入生效（可选但推荐）
```
...
- ✅ **配置走接口**：所有项目配置的读写统一通过 `get-config.sh` / `update-config.sh`。
