#!/usr/bin/env bash
# ============================================================
# search-skills.sh — 搜索平台已注册的可用技能/SKILL
# ============================================================
#
# 用法:
#   ./scripts/search-skills.sh [OPTIONS]
#
# Options:
...
if [[ -z "${PLATFORM_BASE_URL:-}" ]]; then
    ...
    echo "请在运行环境中设置: export PLATFORM_BASE_URL=<平台API基础地址>" >&2
    exit 2
fi

if [[ -z "${SANDBOX_ACCESS_KEY:-}" ]]; then
    echo "[ERROR] 环境变量 SANDBOX_ACCESS_KEY 未设置。" >&2
    echo "请在运行环境中设置: export SANDBOX_ACCESS_KEY=<认证Token>" >&2
    exit 2
fi

if [[ -z "${DEV_SPACE_ID:-}" ]]; then
    echo "[ERROR] 环境变量 DEV_SPACE_ID 未设置。" >&2
    echo "请在运行环境中设置: export DEV_SPACE_ID=<开发空间ID>" >&2
    exit 2
fi
