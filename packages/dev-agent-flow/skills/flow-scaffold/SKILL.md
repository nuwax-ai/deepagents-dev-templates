---
name: flow-scaffold
description: "deepagents-flow-ts 的一句话生成 Agent 脚手架：7 拓扑积木（react-tools/human-in-loop/project-manager/travel-planner/rag/deep-research/dev-agent）+ spec → 生成可跑薄封装 flow + 自动注册 + 自带 typecheck+graph 验证。一句话需求优先用它选拓扑+填槽，而非从零写图。需要把一句话/简单需求快速落地成可跑 flow 时使用。"
tags: [deepagents-flow-ts, scaffold, topology, generate, one-shot, template, coze, building-blocks]
version: "1.0.0"
license: MIT
---

# Flow 脚手架（一句话 → 可跑 Agent · 7 拓扑积木）
...
> 生成的是**真实可读、可手改、可被 inspector 可视化**的 TS（贴合「图是契约」范式，不引运行时解释层）。图逻辑单一权威在 `src/libs/topologies/<name>/`（dev-agent 在 `src/app/topologies/`），生成物只是薄壳。
...
| 场景 | 用 |
|------|-----|
| 需求命中 7 拓扑之一（客服/审批/规划/调研/检索/研究/综合助手） | **flow-scaffold**（生成 + 自验） |
