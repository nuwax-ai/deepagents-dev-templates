...
要 subagent/委派？→ part6 → 平台 或 agents/builtin/（禁止 .agents/agents/）
要 skill？→ part7 → 平台 toolkit 或 skills/builtin/（禁止 .agents/skills/）
```
...
- **禁止写 `.agents/`**：子智能体 → `agents/builtin/` 或平台；技能 → `skills/builtin/` 或平台（Part 6、Part 7）。
