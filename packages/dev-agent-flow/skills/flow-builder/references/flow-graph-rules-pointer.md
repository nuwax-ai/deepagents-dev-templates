...
> **单一权威在目标项目** `docs/flow-graph-rules.md`（规则 ID：`R-G001`…，可持续追加）。
> 目标项目文档只描述模板工作目录本身；开发 Agent 的工作流（何时读、如何验证）由本技能 Part 1–4 补充。
...
| R-G002 | 入口 LLM 容忍非预期输入 | SHOULD | 手测 / `SMOKE_PROMPT_EDGE`（见 part4b-smoke-acp） |
...
| Part 3 工具 | 平台能力登记 → `dev-engineer-toolkit`；**ACP MCP 下发**；**联网搜索**见 § 联网搜索；仓库内分层见目标项目 `docs/capabilities.md` |
...
目标项目含 `.cursor/rules/flow-graph-rules.mdc`（编辑 `graph.ts` / `*.flow.json` 时注入 R-G* 摘要）。
