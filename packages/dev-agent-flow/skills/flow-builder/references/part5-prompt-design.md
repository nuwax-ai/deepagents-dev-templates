...
## 用户输入提炼与平台同步（persona 事务）

system-prompt `<SESSION_CLOSE>` 的**完整步骤**在本节。事务分 **提炼 → 定稿 → 同步**；默认无需用户确认。

### 提炼铁律

用户在本会话**发送的一切**与目标 Agent 相关的内容，须**汇总提炼**为结构化 persona 并同步平台 —— **不是**原样粘贴聊天记录，而是按下方七要素归纳。
...
**持续合并**：每收到相关消息 → 与已有定稿合并；碎片信息 → 主动补全七要素；只描述图/工具未写 persona → **反推**最小 `systemPrompt`（角色+能力+约束），禁止留空。
...
|----|------|
| 1 | 汇总本会话用户输入并提炼（上表落点） |
| 2 | 确认主 Agent（`<AGENT_INTENT_DISAMBIGUATION>`） |
