...
description: "用 LangGraph 开发智能体时使用：本技能封装开发阶段的平台配置操作——查询/搜索/添加/删除工具、保存系统提示词与开场白。这些 4sandbox/agent/dev/* 接口仅在开发期手动运行（取关键数据、更新配置），不进 LangGraph 业务代码。核心流程：先搜索可用工具（Plugin/Workflow/Knowledge），把选定的工具添加到 Agent 配置，然后按工具返回的 schema 把「实际要用的工具」写进 LangGraph 代码（@tool + bind_tools）。关键词：langgraph, 智能体开发, 开发期配置, 系统提示词保存, 搜索工具, 添加工具到配置, 按 schema 开发工具, bind_tools, dev space, sandbox。"
tags: [langgraph, agent-development, tool-config, system-prompt, dev-space, sandbox, plugin, workflow, knowledge]
version: "2.0.0"
license: MIT
---

# Agent Tool Configuration for LangGraph Development

用 **LangGraph** 开发智能体时，本技能解决两件事：① 把**系统提示词 / 开场白**保存到开发平台配置；② 把**平台提供的工具**接入 LangGraph。平台工具虽多，但**必须先配置（搜索 → 添加）才能被 LangGraph 框架使用**，随后按工具返回的 `schema` 进行 LangGraph 工具开发。所有配置操作面向 `4sandbox/agent/dev/*` 接口。
...
| 层面 | 是什么 | 何时运行 | 是否写进代码 |
|------|--------|----------|--------------|
| **配置层（dev 接口）** | config / search / add / del / update | **开发阶段**，由人/AI 手动跑，用来取关键数据、更新平台配置 | ❌ **不进 LangGraph 业务代码** |
| **工具层（实际工具）** | 按平台 `schema` 实现的具体工具（`@tool` + `bind_tools`） | **运行时**，由智能体实际调用 | ✅ 只有这一层写进代码 |

> 一句话：**dev 接口是开发期的"配置工具"，不是运行时被调用的工具。** 写进 LangGraph 代码的，只有「实际要用到的工具」本身——按它的 schema 实现，而不是去 import 这些配置管理 API。

## When to Use

## When to Use

**首要场景**：用 LangGraph 开发智能体时，需要让智能体调用平台工具或设置其系统提示词，就加载本技能，严格按 **搜索 → 添加到配置 → 按 schema 进行 LangGraph 工具开发** 的顺序操作。

- 用 LangGraph 开发智能体、需要接入平台工具 → **必须先搜索** dev 空间，不要凭记忆/臆测直接添加。
...
3. **按 schema 进行 LangGraph 工具开发** —— 以工具返回的 `schema`（字符串化 JSON Schema）为准，在 LangGraph 中实现该工具：把参数名/类型/必填项对齐到 schema，再用 `bind_tools` 绑定到 LLM。不要按臆测字段名开发。

> 口诀：**搜得到 → 加进配置 → 按它的 schema 在 LangGraph 里实现**。三步必须一致指向同一个 `targetType` + `targetId`。
...
### 3.5 按 schema 进行 LangGraph 工具开发（硬约束 ②：以平台 schema 为准）

添加到配置后，在 LangGraph 中实现该工具：**以第 2 步拿到的 `schema` 为唯一依据**定义工具的入参，再用 `bind_tools` 绑定到 LLM。先解析 schema 看清字段：
...
- **字段名、类型以 schema 为准**——LangGraph 工具的参数定义必须与平台 schema 对齐，不要臆造参数名。
- `required` 列出的字段**必填**，缺失会导致平台侧调用失败。
- 添加的工具（`targetType`/`targetId`）与 LangGraph 中实现/绑定的工具**必须是同一个**，不要"加 A 调 B"。
- LangGraph 侧示例（`@tool` 定义 + `bind_tools`）见 `references/langgraph-integration.md`。
...
- ❌ **把 dev 接口写进 LangGraph 业务代码**——config/search/add/del/update 是开发期配置操作，运行时不应出现；代码里只该有"实际要用的工具"本身。
- ❌ **未搜索就直接添加工具**，凭记忆猜 `targetId`——接入平台工具时，搜索是必须优先执行的第一步。
- ❌ **搜到了却没添加到配置**，就在 LangGraph 里写工具——未 `tool/add` 进配置，平台侧不会路由调用。
- ❌ **不按 schema 开发 LangGraph 工具**，按臆测的字段名/类型定义 `@tool`——入参必须与搜索结果返回的 `schema` 对齐。
- ❌ **加 A 调 B**：配置中添加的工具与 LangGraph 中实现/绑定的工具 `targetId` 不一致。
...
- ✅ **代码里只写"实际用到的工具"**；配置变更走 dev 接口（开发期手动跑），两者分离。
- ✅ 接入工具三步一致：**搜 → 加 → 按 schema 在 LangGraph 实现**，指向同一 `targetType`+`targetId`。
