...
把一个目标拆成多个方面**并行**处理、聚合，再让用户确认/调整——**nuwax-flow-ts**
能覆盖的又一类需求（对比 RAG 的线性检索、默认图的迭代循环、human-in-loop 的纯审）。
...
| `research` | 对单个 aspect 发一次**真实搜索**（`createMcpRetrievalNode`；搜索 MCP 经 `searchMcp` 注入），`runTool` 透出过程 | **并行 + 真实 MCP + onToolCall** |
...
流动，不污染固定的图 / checkpointer）。surface（acp/cli）接入逻辑完全复用。
...
pnpm example travel -i
pnpm example travel                         # ACP 服务
```

CLI 跑到行程草案会**暂停等你**输入确认/调整意见（同一终端继续输入）。

> **真实接入（无 demo fallback）**：`research` 经 `createMcpRetrievalNode` 调平台登记的搜索能力（`createTravelGraph` 的 `searchMcp` 参数，运行期进入 `allTools`）；未配置则优雅降级。
> `aggregate`/`finalize` 真调大模型——因此需在 `.env` 配模型凭证
> （`ANTHROPIC_API_KEY` / `ANTHROPIC_AUTH_TOKEN` / `OPENAI_API_KEY`，没配会直接报错而非降级）。
> 有 rate limit 的搜索 API 可用 `rateLimited` 把并行搜索串行化错峰（图仍并行）。在 `index.ts` / recipe 填入 `SEARCH_MCP` 即可。

## 测试

[tests/travel.test.ts](tests/travel.test.ts)：**纯函数**（`gather` 解析 / `fanout` 扇出，无凭证恒跑）守住 map-reduce topology；
**真实接入**用例（`skipIf` 无凭证）跑平台搜索 MCP + LLM 整理 + interrupt→resume。
