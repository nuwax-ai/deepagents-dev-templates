...
| `research` | 对单个 aspect 发一次**真实搜索**（createMcpRetrievalNode；搜索 MCP 经 `searchMcp` 注入，duckduckgo 实测不稳定已移除），`runTool` 透出过程 | **并行 + 真实 MCP + onToolCall** |
...
> **真实接入（无 demo fallback）**：`research` 经 `createMcpRetrievalNode` 调注入的搜索 MCP（`createTravelGraph` 的 `searchMcp` 参数；duckduckgo-mcp-server 实测不稳定已移除，需自备可用搜索源，未传则优雅降级）
