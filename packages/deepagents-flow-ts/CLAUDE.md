...
你的职责：
1. 在 **`src/app/`** 改默认图，或在 **`examples/`** 照范例新增 flow
2. 对照 **`examples/`** 找拓扑与挂接方式，再写 `graph.ts` / `nodes/`
3. **不要改** `src/surfaces/`、`src/runtime/` 的接入骨架（除非用户明确要求）
...
src/
  app/           # 默认 ReAct 图（可改）
  runtime/       # FlowRuntime、config、checkpoint（保护；扩展放这里）
  surfaces/      # ACP / CLI 接入（保护）
...
配置加载、模型解析、日志、MCP、平台 API 等由依赖包 **`deepagents-app-ts/runtime`** 提供（`pnpm install` 后可用）。
本项目的 flow 封装在 [src/runtime/flow-runtime.ts](src/runtime/flow-runtime.ts)、[src/runtime/config.ts](src/runtime/config.ts)。

代码中常见 import（扩展或 vendoring 时对照）：

`loadConfig`、`AppConfig`、`AppConfigSchema`、`resolveModel`、`logger`、`createRuntimeContextAsync`、`resolveSystemPrompt`、`resolveSkillsPaths`、`discoverSubAgents`、`RuntimeContext`、`MCPManager`、`ACPSessionConfig`

若 npm 包未覆盖所需能力：把实现 **拷贝进 `src/runtime/`**（或 `vendor/`），import 改为本项目路径 —— **只在当前仓库内解决**。
...
| 区域 | 路径 | 规则 |
|------|------|------|
| 接入 / 引擎 | `src/runtime/`、`src/surfaces/` | 默认不改 |
| 默认图 | `src/app/` | 可改 |
...
1. **图是契约** — 边与条件路由在 `graph.ts`；决策逻辑抽纯函数 + 单测
...
### 两类 Flow（`src/surfaces/flow-types.ts`）
...
- ACP：`deepagents-acp`
- 配置 / 模型 / MCP：`deepagents-app-ts/runtime`
