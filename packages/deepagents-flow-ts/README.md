...
|---|---|---|---|---|
| `react-tools` | 通用工具对话 | `bindTools` + `ToolNode` | [默认图](src/app/graph.ts) | `pnpm flow`（`flow.active: default`） |
...
| `dev-agent` | 综合工具助手 | ReAct + 压缩 | [app/flows/dev-agent](src/app/flows/dev-agent/) | 内置 `flow.active: "dev-agent"` |
