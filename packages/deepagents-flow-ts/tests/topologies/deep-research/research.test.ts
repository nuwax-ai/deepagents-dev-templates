/**
 * 深度研究报告 flow 测试。
 *  - 纯函数（无凭证、确定性）：routeAfter* / fanout / delivery —— 守住双层 reflection + Send 扇出。
 *  - 真实接入（skipIf 无凭证）：多阶段 + 多轮 HITL 闭环。
 *  - durable stateful flow（无凭证）：clarify interrupt 跨 flow 实例仍能 resume。
 */
...
} from "../../../src/libs/topologies/deep-research/index.js";
import {
  artifactParentDir,
  formatDeliveryAnswer,
  markdownToHtml,
  writeDeliveryArtifacts,
} from "../../../src/libs/topologies/deep-research/nodes/index.js";
...
function createResearchFlow(
  ...
  return materializeRecipe<ResearchStateType>(
    {
...
describe("durable stateful flow: clarify interrupt cross-instance resume (no creds, flagship e2e)", () => {
  const dirs: string[] = [];
  afterAll(() => {
