...
（`Cmd+,`），就能在 agent 面板里 chat 调试当前 `activeFlow`。
...
默认 `model.provider` 即 `"openai"`（见 [config/flow-agent.config.json](../config/flow-agent.config.json)）；改用 Anthropic 协议时设为 `"anthropic"`（见文末）。
...
切换 flow 时只需修改 [config/flow-agent.config.json](../config/flow-agent.config.json) 的
`activeFlow` 并重启 Zed agent；所有注册 flow 共用 `src/index.ts` 入口。
...
2. **入口不带子命令 = ACP 模式**：`args` 只给 `src/index.ts` 路径；具体图由 `activeFlow` 选择。
...
flow 必须注入有效凭证。固定管道 topology 经 scaffold 生成并注册后，同样通过 `activeFlow` 调试。
