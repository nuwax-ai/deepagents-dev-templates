...
这让平台侧（ACP host）与工作区配置、环境变量、模板内置各司其职，互不越界。

## 来源层

| 层 | 来源 | 例子 | 平台可配置 |
| --- | --- | --- | --- |
| `acp-dynamic` | ACP 会话 / config | systemPrompt、mcpServers、skills、model、subagents | ✅ |
...
- **mcpServers** — 两层合并（`config/mcp.default.json` < ACP/session MCP，`session-wins`），native 工具经 `@langchain/mcp-adapters`（`MultiServerMCPClient`）由 runtime-context 加载。
...
- **加 MCP**：编辑 `config/mcp.default.json`，或经 ACP `session/new` 的 `mcpServers` / 平台启动时注入的 `ACP_SESSION_CONFIG_JSON.mcpServers`。常用 server 见 [config/mcp.examples.json](../config/mcp.examples.json)（context7 / chrome-devtools / filesystem / bash）。
